...
export const fetchCoins = async() =>{
  ...
  const response = await axios.get(`${BaseUrl}/coins`).then(res => res.data);
  return response;
}
// coin 화면 api
export const coinUrl = async(coinId:string) =>{
  const response = await axios.get(`${BaseUrl}/coins/${coinId}`).then(res => res.data);
  return response;
}
export const priceUrl = async(coinId:string) =>{
  const response = await axios.get(`${BaseUrl}/tickers/${coinId}`).then(res => res.data);
  return response;
}
// 차트 화면
